...
check_send_quota() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        echo -e "\e[33m[INFO]\e[39m Max 24h quota sending pour ${AWS_DEFAULT_REGION}: $quota Emails."
        echo -e "${AWS_ACCESS_KEY_ID}:${AWS_SECRET_ACCESS_KEY}:${AWS_DEFAULT_REGION} ($quota per day)" >> smtp/aws_smtp.txt
        if [ $quota -ge 10000 ]; then
            echo -e "\e[92m[OK]\e[39m Creation of SMTP identifiers in progress..."
            ...
            if [[ $? -eq 0 ]]; then
                ...
                if ! grep -q "SHUTDOWN" <<< $salut; then
                    echo -e "\e[92m[OK]\e[39m ${AWS_ACCESS_KEY_ID} [$quota mail/day] Work !"
                    echo -e "$output" >> smtp/healthy_aws_smtp.txt
                else
                    echo -e "\e[91m[FAIL]\e[39m ${AWS_ACCESS_KEY_ID} [$quota mail/day] Doesn't work"
                fi
            fi
        fi
    fi
